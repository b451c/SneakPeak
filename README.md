...
- **Buffer only on demand** - tools that need raw samples (Spectral view, One-Shot Factory, Paste) load the buffer in the background with progress in the title (the Hard Limiter opens on long items too and limits the file itself); a buffer over 1 GB is refused with a message instead of being allocated.
