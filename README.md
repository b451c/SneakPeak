...
- **One-Shot Factory** (new in v2.4) - the SFX-prep chain in one panel (right-click > One-Shot Factory...): trim silence (threshold + padding), edge micro-fades, normalize (Peak / LUFS-I / True-peak safe via the limiter), with a live prep preview on the waveform. Run exports `{name}_01.wav` next to the source; the loaded audio is never touched.
