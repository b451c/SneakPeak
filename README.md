...
- **Settings panel** - gear icon in the mode bar: UI scale with live preview, density presets (Compact / Comfortable / Spacious), Fit to Window, and the Ruler / Meters / View preferences - including hide-ruler and the Detailed/Simple waveform style (new in v2.4). The panel stays open while you work and can be dragged anywhere over the waveform and spectral panes (v2.5).
...
- **Buffer only on demand** - tools that need raw samples (Spectral view, One-Shot Factory, Paste) load the buffer in the background with progress in the title (the Hard Limiter opens on long items too and limits the file itself - over 5 minutes its live preview waits for the PREVIEW pill, so nothing is decoded until you ask); a buffer over 1 GB is refused with a message instead of being allocated.
...
### Dynamics Processing
Right-click > Process > Dynamics Panel. SneakPeak auto-activates the take volume envelope if it isn't already enabled (no manual setup). Analyzes audio and writes volume envelope automation - zero CPU cost during playback. Since v2.5 the analysis streams the source at full rate on a worker thread: knobs track the mouse on items of any length (4-10 ms per move on a 20-minute item) and the first pass shows its progress in the title. In Standalone mode (new in v2.4) the same panel applies destructively instead - the gain curve (comp + gate + de-ess) is multiplied into the audio with one undo step, completing the standalone podcast chain (Dynamics > Hard Limiter > Save).
...
- **True-Peak Hard Limiter** (new in v2.4) - transparent lookahead brickwall limiting with a hard **dBTP** ceiling (8x-oversampled detection + output re-measure, passes BS.1770-4 meters with margin). Premium panel (Gain/Ceiling/Attack/Hold/Release, TRUE PEAK + LINK, 4 presets incl. Game Asset -1 dBTP), live GR band over the waveform, threaded preview readouts (optional on audio over 5 minutes: a PREVIEW pill starts the pass, the choice is remembered), whole-file or selection apply with undo. Works on Standalone files AND directly on a selected timeline item (since v2.5 an in-place rewrite of exactly the item's window of its source file - trimmed items and long items included, an MP3/FLAC/AAC source converted to WAV first after one question - with a Reverse-style confirm + undo). Right-click > Process > Hard Limiter...
