...
REAPER is the most powerful DAW on the market, but its arrange view wasn't designed for sample-level audio editing. SneakPeak fills that gap by providing a dedicated waveform item editor that lives inside REAPER as a native extension.

I built SneakPeak because I needed it in my own workflow — and I decided to share it with the community for free, hoping that those who find it useful will consider supporting its development. I could have made it a paid extension, but ultimately I believe the REAPER community deserves great tools that are accessible to everyone.
...
### Waveform Display
- **Precision waveform rendering** — Peak + RMS display with dB scale, clip indicators, and zero-crossing line. Powered by REAPER's SDK peak rendering for zero-RAM waveform display.
...
src/
  ...
  waveform_view.h/cpp   Peak rendering, zoom, selection, coordinate conversion, SDK peaks
  ...
  minimap_view.h        Resizable minimap overview with SDK peak fallback
...
The extension uses REAPER's SDK peak rendering (`GetMediaItemTake_Peaks`) for zero-RAM waveform display, with lazy audio loading on demand for destructive operations, spectral analysis, and clipboard ops. Double-buffered GDI rendering ensures smooth, flicker-free display.
