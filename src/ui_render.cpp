...
// Embedded UI font (generated by CMake from src/assets/fonts/Inter-Medium.ttf).
...
// Cached face for the embedded Inter Medium (created once, lives for the process).
static const BLFontFace& InterMediumFace() {
  static BLFontFace face = [] {
    BLFontData fd;
    fd.create_from_data(g_fontInterMedium, g_fontInterMedium_size);
    BLFontFace f;
                                   ...
                                   const DynCurveParams& p)
{
  ...
  if (!img) return;
  {
    ...
    // input-axis dB tick labels (embedded Inter, crisp at any DPI)
    {
      BLFont f;
      if (f.create_from_face(InterMediumFace(), (float)dynui::kFsTick) == BL_SUCCESS) {
        const double ticks[] = { -48.0, -36.0, -24.0, -12.0, 0.0 };
        for (double db : ticks) {
          char buf[8];
