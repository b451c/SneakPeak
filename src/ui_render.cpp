...
bool UiCanvas::ensure(HDC hdc, int devW, int devH)
{
  ...
  m_panelKey = 0;   // the surface is recreated below: no raster in it
...
// FNV-1a over every field the renderer reads (strings by content, static label
// pointers by identity) plus the device size. A field added to DynPanelVM or
// KnobVM must be added here, or the panel will not repaint when it changes.
uint64_t UiCanvas::HashPanelVM(const DynPanelVM& vm, int devW, int devH)
{
  uint64_t hsh = 1469598103934665603ull;
  auto bytes = [&](const void* p, size_t n) {
    const unsigned char* bp = (const unsigned char*)p;
    for (size_t k = 0; k < n; k++) { hsh ^= bp[k]; hsh *= 1099511628211ull; }
  };
  auto d = [&](double v) { bytes(&v, sizeof(v)); };
  auto i = [&](int v) { bytes(&v, sizeof(v)); };
  auto b = [&](bool v) { unsigned char cb = v ? 1 : 0; bytes(&cb, 1); };
  auto str = [&](const char* sv) { if (sv) bytes(sv, strlen(sv)); i(sv ? 1 : 0); };
  auto ptr = [&](const void* p) { bytes(&p, sizeof(p)); };
  const DynCurveParams& c = vm.curve;
  d(c.thresholdDb); d(c.ratio); d(c.kneeDb); d(c.gateThreshDb); d(c.gateRangeDb); d(c.gateRatio);
  d(c.gateHystDb); i(c.mode); d(c.maxBoostDb); b(c.compBypass); d(c.makeupDb); d(c.avgPeakDb);
  d(c.avgGrDb); d(c.inMinDb); d(c.inMaxDb); b(c.showGate);
  i(vm.activeTab); str(vm.presetName);
  for (int k = 0; k < kDynNumParams; k++) {
    const KnobVM& kv = vm.knobs[k];
    d(kv.value); d(kv.norm); d(kv.defaultNorm); ptr(kv.label); ptr(kv.unit); i(kv.precision);
    b(kv.isGate); b(kv.showAuto); b(kv.showOff); b(kv.showAutoThr); b(kv.showInf); b(kv.hover);
    b(kv.editing); str(kv.editText); b(kv.caretOn);
  }
                           ...
                           const DynPanelVM& vm)
{
  ...
  const uint64_t key = HashPanelVM(vm, devW, devH);
  if (key == m_panelKey) {   // same raster already in the surface: blit only
    StretchBlt(hdc, x, y, w, h, m_memDC, 0, 0, devW, devH, SRCCOPY);
    return;
  }
  m_panelKey = 0;
