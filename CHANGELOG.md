...
- **Windows: a destructive edit right after a source swap or a fresh insert failed with an error box** - REAPER brings media online a moment after the item gets its source (the WAV of "convert MP3 to WAV", a file just dropped in), and the file reader it opens then outlives the "media offline" call the edit makes first, so the in-place open failed with "Write failed and the file could NOT be restored" although nothing had been written. The edit now takes the item offline again while it waits for the file ("waiting for REAPER to release the file (Esc cancels)" in the title; Esc leaves the file untouched).
