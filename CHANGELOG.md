...
- **Everything works on the source at its own sample rate.** Dynamics, exports, Copy, destructive edits and peak Normalize on long items used to go through a reduced-rate working buffer; they now stream the file itself.
- **A full audit, fixed in one go:** destructive-edit safety (undo copies, trimmed and reversed takes, in-place rewrites), Standalone, Windows paths and temp folders, WAV format handling, keyboard passthrough - fifty-odd fixes below.
...
- **Save as an action** - "SneakPeak: Save file (Standalone)" saves the active Standalone file exactly like Ctrl+S (bindable in the Action List, callable from scripts; no-op outside Standalone).
- **Split-band de-esser (Standalone)** - the DE-ESS tab gains a WIDE / SPLIT switch in the footer. In Standalone, SPLIT makes Apply cut only the detector band during a sibilant instead of ducking the whole signal: the band is the one the BP / HP detector listens to (what LISTEN paints is what gets cut), the cut depth follows the same threshold, ratio, range and attack/release, and it is applied zero-phase so it never boosts and leaves the signal untouched wherever the de-esser is idle. Everything else in the chain (compressor, gate, makeup) still applies as a wideband gain in the same pass, with one undo step. Items keep the wideband envelope de-esser (an envelope cannot split), so the switch renders disabled there.
- **Lanes (per Track) with per-track zoom (Multi-item view)** - a fourth display mode next to Mix and the two Layered modes: every track gets its own horizontal lane, stacked in arrange order, labeled with the track name and carrying its own dB scale. Scroll on a lane's dB scale (or Option/Alt+scroll over the lane) zooms that track alone, so one track can be blown up for detail while the others keep their context; the lane label shows the factor ("x2.3"), the toolbar's V Zoom +/- still scale every lane together and V Zoom reset returns them all to 1:1. A stereo lane splits into L/R bands when it is tall enough and folds both channels into one band otherwise. The per-track zoom survives the rebuilds the view does after edits made outside SneakPeak (forum #68). Layered (per Track) colors now follow the track order as well (track 1 is always green), not the order of the item positions.
- **Spectrogram FFT size** - Settings > View gains **FFT 512 / 1024 / 2048 / 4096** (default 2048, the size the spectrogram always used). A larger size resolves close partials and hum harmonics (4096 is about 11 Hz per bin at 44.1 kHz), a smaller one keeps clicks and transients sharp in time (512 is about 3 ms per column); the overlap stays at 75% at every size, so memory and compute time do not change with it. The setting is remembered, and a change recomputes the open spectrogram on the spot. Spectral Repair keeps its own fixed analysis size.
- **What you had open on an item comes back** - the spectral view, the zoom, the cursor and the selection of a timeline item are remembered while REAPER runs: click another item and back, and you continue where you were. The panels (Dynamics, Hard Limiter, Settings) stay open across items as before.

### Changed
- **Long items no longer freeze REAPER** - selecting an item paints the waveform from REAPER's own peak files at once, the same data the arrange view draws, so what you see matches REAPER at every zoom; Timeline view after an edit, SET view and Multi-item view do the same instead of decoding every segment or layer synchronously. Items under about 3.8 minutes (stereo, 44.1 kHz) then load their full-rate working buffer in the background, as before. Timeline view also drops its old 600-second span limit: a long item split by a delete stays in Timeline view instead of falling back to a full reload. Measured on a 20-minute WAV and a 17-minute AAC: the longest main-thread stall on select, reselect, delete and multi-select went from 0.4-1.5 s to under 0.1 s. One consequence on short items: a sample-based command issued within the first ~100 ms after selecting, before the buffer has landed, shows a "still loading" toast and works on the next try (v2.4.0 blocked until the decode was done).
- **Long items hold no working buffer until something needs the samples** - since the waveform, the minimap, every export and the Dynamics panel work from REAPER's peak files or stream the source, the reduced-rate buffer of an item over about 3.8 minutes is decoded only on demand: the Spectral view and the One-Shot Factory open at once and fill in when it lands (the title shows the progress), Paste asks for it with a short toast and works once it has loaded, and the Hard Limiter opens on them at once (its readouts fill in when the buffer lands; Apply itself limits the file). Selecting a one-hour item, re-selecting it, deleting a range of it in Timeline view or entering a SET view over long items now costs no decode and no memory (a one-hour stereo item used to hold 460 MB), and an item whose buffer would exceed 1 GB is refused with a message instead of silently allocated. The level meter follows playback on such items straight from the source. The RMS band needs real samples and stays off on these items; full-rate items are unchanged.
- **Destructive edits run in the background** - Reverse, DC Remove and Gain on a selection used to hold REAPER from the confirmation prompt until the file was rewritten, with the pre-edit copy written first in the same freeze: on a slow or busy disk (a network share, a spinning drive, a file not yet in the system cache) a long item meant seconds of a frozen REAPER with no way out. The rewrite, its pre-edit copy and any rollback now run on a worker thread: the title shows the progress ("Reversing... N% (Esc cancels)"), REAPER keeps playing and painting, and Esc cancels the edit and puts the pre-edit copy back byte for byte. Until the write lands the edited item stays on screen (a new selection takes effect the moment it does), and commands that would read or rewrite the same file meanwhile - Copy, Normalize, exports, Delete/Split/Paste on the item, another destructive edit, Undo - answer with a short "in progress" message instead. Measured on a 20-minute 24-bit WAV already in the system cache (the best case for the old path): the longest main-thread stall after the prompt went from 0.06-0.27 s to under 0.07 s; on an uncached file the old stall grew with the file size, the new one does not.
...
- **Dynamics on items streams the source at its own rate** - the Dynamics panel (compressor, gate, de-esser, Live, Apply) used to analyze the working buffer, which on items over about 3.8 minutes is band-limited with flattened peaks: on a one-hour item a one-sample click at 0.9 read 0.16 and a 12 kHz sibilant tone all but vanished. Item views now analyze the source at its own rate, streamed chunk by chunk in the background (the title shows "Analyzing dynamics... N%"); on a short item this takes a few tens of milliseconds and nothing visible changes, on a long one the curve, the GR meter and a pressed Apply follow as soon as the analysis lands. Changing the detector (Peak/RMS, RMS window, de-ess band) re-streams; every other knob recomputes from the finished analysis. Standalone files are unchanged (full-rate buffer, analyzed synchronously). The analysis also holds a third of the memory it used to per item.
- **Dynamics knobs track the mouse on long items** - the analysis no longer runs inline on every mouse move: the panel updates at input rate while the engine recomputes the latest knob position on a worker thread, and Live mode writes the envelope 150 ms after the knob settles (and immediately on release), so the written points for the final position are unchanged. Measured on a 20-minute item with Live on: 10 ms per mouse move (was 230 ms). The envelope ceiling lookup and the transfer-curve overlay are cached and clipped to the visible range.
- **Apply Dynamics on long items no longer stalls REAPER, and keeps the envelope under 20,000 points** - the curve written to the envelope is simplified from every analysis point (3.6 million for an hour of audio); that simplification now runs on the analysis worker right after the analysis instead of on the main thread when Apply is pressed, so Apply and Live writes only insert the ready curve (measured on a one-hour WAV: longest main-thread stall on Apply 0.70 s -> 0.05 s, identical points). An hour of busy material still simplified to about 30,000 points at the 0.3 dB tolerance, more than REAPER's envelope editing handles comfortably, so the curve is coarsened step by step (up to a 2 dB bound) until it is under budget and the toast reports the resulting tolerance ("Envelope simplified to N points (tolerance X dB)"). Short and normal items are unchanged. A click or key while the worker is still busy no longer waits for it on item views.
- **De-esser analysis no longer grinds through long silences on x86** - the sidechain filter's state never reaches zero in a silence by itself (it settles on the smallest representable number), and x86 processors handle such values through slow microcode, so the background analysis of material with long pauses could take many times longer than it should. The filter state is now flushed to zero below -600 dB and the analysis thread runs with flush-to-zero set; the envelope output is unchanged.
...
- **Destructive edits ask first and refuse first** - Gain +3 / -3 dB on a selection rewrites the selected range of the item's source file exactly like Reverse and DC Remove, but was the only such command that never asked: it now shows the same "modifies the audio file on disk. Continue?" prompt. And no destructive command asks a question it will then refuse: a non-WAV source (MP3, FLAC, ...), a take reversed or sectioned in REAPER, or an edit already running used to be reported AFTER the Yes (a second "WAV only" box), under the panel (a toast), or not at all (Hard Limiter... on a long item did nothing). Every entry now checks before the prompt: the Process menu rows that rewrite the file are labeled "(rewrites file)" and grey out with the reason on a row of their own ("Cannot rewrite the file: the source is MP3 - WAV only (use Edit Copy)"), and the Hard Limiter panel's footer shows the reason with a greyed Apply.
- **Hard Limiter on items limits the item's window of the file** - Apply on a timeline item used to need the item to cover its whole source file untrimmed, at a normal channel mode, under about 3.8 minutes - which is almost never true of a real item, so most attempts ended in "needs the whole file" or "too long". It now edits the file in place like Reverse, Gain and DC Remove: exactly the item's window (a time selection limits that range, with 20 ms handoff ramps into the untouched audio), all of the file's channels (a 6-channel file stays 6-channel, a mono-mode take keeps its stereo file), items of any length with a lazy or downsampled working buffer included, in the background with "Limiting... N% (Esc cancels)" in the title and the pre-edit copy as the one-level undo. One range at a time is held in memory for the limiter's passes - about 6 minutes of stereo at 44.1 kHz, 25 minutes of mono at 22 kHz - and a longer range greys Apply with the limit for that file. Nothing above the ceiling with 0 dB gain leaves the file, the undo slot and REAPER's undo history alone.
- **Hard Limiter preview is optional on long audio** - the panel used to start its full preview pass the moment it opened, whatever the length: on a long file that meant a busy CPU and a wait for readouts you may not have wanted. Audio over 5 minutes now opens the panel without a pass - readouts blank, no gain-reduction band - and a PREVIEW pill in the panel header starts it, with the progress on the pill; a second press drops the preview again and the choice is remembered for long files. A long timeline item is not even decoded until the pill is pressed (Apply limits the file itself either way). Shorter audio previews at once, as before, and shows no pill.
- **Destructive edits on MP3, FLAC and AAC items convert the source first** - a compressed file has no samples to rewrite in place, so Reverse, DC Offset Remove, Gain on a selection and the Hard Limiter used to refuse such items ("WAV only - use Edit Copy"). They now ask one question and do the whole job: the file is decoded to a WAV next to it (`name.wav`, 32-bit float, in the background with "Converting to WAV... N%" in the title, Esc cancels and leaves nothing behind), every item that used the compressed file is pointed at the WAV (one REAPER undo point; the original is never touched), and the edit you asked for runs on the WAV with no second prompt. The Process menu rows read "(converts MP3 to WAV)" and the Hard Limiter footer says the same with Apply enabled. From then on the item is a WAV item and every later edit is instant and in place.
- **The Dynamics panel says what Apply does** - a footer line reads "APPLY WRITES THE TAKE VOLUME ENVELOPE" on items and "APPLY EDITS THE BUFFER - SAVE WRITES THE FILE" in Standalone; on the DE-ESS tab of an item it adds that SPLIT needs Standalone (the WIDE / SPLIT pair renders disabled there). The panel is 14 px taller for the line.
- **The Settings panel stays open and can be moved** - any click outside the panel used to close it (and the click went nowhere), and the panel itself sat welded to one place, squeezed into the waveform pane when the spectral view was open. It now stays open until the gear, its X or Escape closes it (a click outside goes to whatever is under it), drags like the other premium panels from any free spot on it (the place is remembered), and floats over the whole waveform + spectral area at its full size.
- **Minimap in the View menu** - right-click > View > Minimap toggles the overview in the premium build too (it was a Settings-only pill there; the guide already said the menu had it). The Settings > VIEW pill stays.

### Fixed
- **The gain knob floated above the panels** - the little gain bubble was drawn over the Hard Limiter, Dynamics and One-Shot panels and took the clicks meant for them. It now sits under every panel (still above the waveform).
- **The spectrogram ignored the amplitude** - the item volume (the gain knob in ITEM mode) scaled the waveform but left the spectrogram unchanged, and in Standalone a gain baked from the knob left the old spectrogram on screen (Gain, Normalize and Reverse were already covered). The spectrogram's colours now follow the item volume and the knob's live preview exactly as the waveform does, and a knob gain baked into a Standalone buffer recomputes it.
- **Standalone: soloing a channel stopped playback** - the L/R badge went through the take's pan, which a Standalone file does not have, and the click itself stopped the preview like any other click. The badge now keeps the preview playing: it restarts in place with that channel silent (the file's channel layout kept), and the second click brings it back the same way.
- **Windows: a destructive edit right after a source swap or a fresh insert failed with an error box** - REAPER keeps a source file open while its peak builder reads it and for a moment after the item comes online, so an in-place edit started in that window (the Hard Limiter straight after "convert MP3 to WAV", Reverse on a file just dropped in) could not open the file and ended in "Write failed and the file could NOT be restored" although nothing had been written. The edit now waits for REAPER to let go of the file, with "waiting for REAPER to release the file (Esc cancels)" in the title meanwhile (a 22-minute file's peak build takes a few seconds; Esc leaves the file untouched).
- **32-bit integer WAVs are edited in place and saved as they are** - the in-place editor (Reverse, DC Remove, Gain on a selection, the Hard Limiter on items) knew 16/24-bit PCM and 32-bit float only, so a 32-bit integer WAV (many field recorders) passed the confirmation and then failed with "Write failed - the file was restored"; Standalone's Save silently wrote such a file back as 16-bit. Both handle 32-bit integer PCM now, and a WAV the editor still cannot rewrite (8-bit, ADPCM) is refused before the prompt, from the file's header, with the depth in the message.
- **The info line under a Standalone file showed the last item's format and size** - the sample rate, channel count and file size in the bottom panel came from the take that was selected before the file was opened (a 48 kHz WAV dropped in after a 22 kHz MP3 item read "22050Hz ... 10.3MB"); a Standalone open and a tab switch now report the file itself.
...
- **Exports from long items were written at a reduced sample rate** - drag export, "Edit Copy in Standalone" and the One-Shot Factory all wrote the downsampled working buffer of an item over about 3.8 minutes, so a 5-minute item produced 22050 Hz files (an hour-long one, 8000 Hz). These exports now stream the item from its source at the source's own rate, chunk by chunk, with no whole-item buffer: drag export writes the selection in the source file's format (24-bit PCM for non-WAV sources), Edit Copy writes the copy in the background with progress in the title (an item whose full-rate copy would not fit Standalone's 1 GB buffer is refused with a message), and each One-Shot slice is trimmed, faded and normalized on the full-rate audio (a slice over 3.8 minutes is skipped - one-shots are short). Loop Lab is unaffected (it only ever works on Standalone's full-rate buffer).
- **Copy from a long item put reduced-rate audio on the clipboard** - Copy and Cut lifted the selection out of the downsampled buffer, so Paste then inserted an 8 kHz temp file into the project. Copy now streams the selection from the source at its own rate and needs no working buffer; a selection whose samples would not fit the 1 GB buffer cap is refused with a message. Standalone and Multi-item copies are unchanged.
- **Destructive edits on long items could re-encode the source at a reduced sample rate** - Reverse, DC Remove and Gain on a selection wrote the downsampled buffer back to the file (a 20-minute WAV came back at 8 kHz), as did Undo of such an edit. Those three edits now stream through the file itself at its own rate, in place, with every other chunk left untouched, and the single-level destructive Undo snapshots the pre-edit file itself (a temp-folder copy) rather than the working buffer, so it restores the original bytes on items of any length.
- **Normalize (peak) on long items landed too hot** - the peak was measured from the downsampled buffer, which flattens transients (a full-scale click can read 15 dB low), so the item was normalized above the -0.1 dB target. Peak normalize now streams the real peak at the source's full sample rate (no buffer needed at all, so the "loading" wait on long items is gone with it); LUFS normalize was always measured by REAPER and is unchanged.
- **Snap to Zero-Crossing did nothing on long items** - the snap searched the working buffer, which a long item no longer has or holds at a reduced rate, so the selection edges stayed where the mouse was (or on the reduced grid). The edges are now looked up in a small window read from the source at its own rate: sample-accurate on items of any length.

#### Destructive edits in ITEM mode
...
- **Items locked in REAPER could be deleted** - SneakPeak's Delete went through REAPER's API, which refuses to split a locked item but deletes one whole without complaint, so a selection covering a locked item removed it (and the ripple moved locked neighbors). Locked items are now left alone by Delete, ripple delete, Paste and Gain on a selection, with a message: "Item is locked in REAPER - unlock it to edit here".

#### Standalone
- **Standalone silently folded multichannel files to stereo** - a 6-channel WAV dropped into Standalone loaded as its first two channels, and Save wrote that back over the original (channels 3-6 gone). Standalone now refuses files with more than two channels up front, with a message; the same 1 GB working-buffer limit as for items applies to Standalone too (a file over it is refused instead of allocated, and very long files no longer overflow the frame count).
...
- **Standalone Dynamics could keep analyzing the audio from before a small edit** - the analysis is cached behind a sparse fingerprint of the buffer (every 4096th sample), so a short in-place edit that fell between fingerprinted samples (a silenced syllable, a pencil stroke) left the old analysis in place and the next Apply compressed with the curve of the unedited audio. Every Standalone edit now drops the cached analysis outright.
- **Standalone Dynamics could race its own buffer** - knob drags in Standalone were routed through the background analysis worker although Standalone edits replace the buffer on the main thread; Standalone analyzes synchronously again (as documented), and every buffer mutator waits for a running worker first.
...
- **Saved Dynamics settings are range-checked** - a parameter string from an item (P_EXT) or a preset with a garbled or out-of-range value (a hand-edited file, a future version's keys) could put a knob outside its range or turn a garbled ratio into Inf:1; every value is now clamped to its knob range and a value that is not a number keeps the default.
- **De-Ess threshold knob: "Auto" at hard-left** - the engine's automatic band threshold (the band's average level) existed but no knob position reached it; like the gate threshold's "Off", dragging D.Thr fully left now snaps to Auto and the readout says so.
- **Typing a value with a comma** - keyboards with a comma decimal key ("3,5") now enter the value correctly in the Dynamics panel's inline editor.
- **Guide: the Dynamics knob table now matches the knobs** - Knee, Attack and Release were listed with ranges the knobs never had (0-30 dB, 0.1-200 ms, 5-2000 ms; they are 0-24 dB, 0-500 ms, 0-1000 ms) and the seven DE-ESS knobs were missing; a repository check keeps the table equal to the code from now on.
- **REAPER's audio-accessor API is never entered from two threads at once** - the background analysis reads its accessor on a worker while the main thread creates, polls and destroys others (item loads, change detection, exports); no problem was ever observed, but the SDK gives no guarantee for that overlap, so every accessor call now goes through one lock (held per call, never across a read loop; measured: no change to the main-thread stall under a 20x select stress with the Dynamics panel open).
...
- **Copy in Multi-item view copied nothing** - the selection range was clamped to the shared buffer's sample count, which the Multi-item view never has (every layer keeps its own samples), so Copy and Cut returned before touching the clipboard, and Paste then inserted the previous clipboard, or nothing. Copy now takes the mix of every layer inside the selection (a mono item feeds both channels of a stereo mix), as documented.
...
- **REAPER could crash on the next paint after a script deleted an item without an undo point** - Timeline, SET and Multi-item views keep the takes they draw; when a script removed one of those items with a bare API call (no undo point, so REAPER's project-state counter never moved and the view was not rebuilt), the next paint asked REAPER for the envelope of the freed take, and once that memory had been reused REAPER crashed. Freed takes are now dropped before every paint and the view is rebuilt from the project on the next tick.
- **A working set could adopt a new track** - the SET view remembered its track by address; after the project was cleared and a new track allocated at the same address (REAPER reuses freed memory), the old set claimed the new track's items and kept the view locked. The set now also remembers the track's GUID and lets go when it changes.
- **A working set could lock the view** - after a SET view whose items (or track) were later deleted in REAPER, SneakPeak could refuse to load any newly selected item (stale set flags compared dead item pointers; a new item reusing the address counted as "in the set"). The set is now validated before it is consulted and reset when nothing of it survives.
...
#### Windows
- **Dragging audio out of SneakPeak did nothing** (forum #105/#97, Ben Zero) - the exported selection was written, but the Windows shell refused the file's path (SneakPeak builds paths with forward slashes, which the C runtime accepts and the shell's item parser rejects), so the drag session never started - into REAPER's timeline or anywhere else. The path handed to the shell is now normalized, and a drag from a selection drops into the arrange as a new item.
- **Tiny UI on scaled displays despite a persisted 1.0 scale** (forum #97/#105, Ben Zero) - an install that saved UI scale 1.0 before the first-run auto-detect existed (v2.2.0), or detected it on a 96-DPI monitor, kept the tiny scale on a scaled display forever. If the saved scale is exactly 1.0, the user never moved the UI SCALE slider, and the system asks for 125% or more, the scale now re-seeds from the system once; a slider choice still ends the automatics for good.
- **Standalone playback was silent, and Paste, Undo snapshots, One-Shot LUFS normalize and some exports failed quietly** (forum #105, Ben Zero) - every temporary WAV SneakPeak writes (the Standalone preview file, the paste clip, the pre-edit undo copy, the One-Shot LUFS measurement file, the export fallback when a project is unsaved) went to `/tmp`, which does not exist on Windows, so the write failed and the feature did nothing. They now go to the system temp folder (`%TEMP%` on Windows; `$TMPDIR` or `/tmp` elsewhere, unchanged). Export paths also recognize Windows path separators, so "next to the source file" works there too.
- **Destructive edits failed on files under a non-ASCII path** - the in-place editors (Reverse, DC Remove, Gain on a selection) and the loop-point reader opened the file through the ANSI C runtime, so a source under a folder or user name with accented characters was "not found" ("Failed to write WAV file"). They now open files the same UTF-8-aware way as the rest of SneakPeak.
- **Temporary files leaked under a non-ASCII user name** - the undo copies, paste clips, preview and export temp files were deleted with the ANSI C runtime, which does not find them under an accented %TEMP% path; they are now removed through the UTF-8 file API.
- **Undo of a destructive edit failed with "check the file permissions"** - after an in-place edit the old media source kept a write-denying handle on the file for the rest of the session, so restoring the pre-edit snapshot (and any later destructive edit of the same file) was refused. The post-edit refresh no longer replaces the source object on Windows (the file is rewritten in place, so REAPER picks the new audio up from the same source), and the restore itself now takes the item's media offline for the copy, like the destructive edits themselves.
- **The floating window opened at twice its size** - the window template gives its size in dialog units, which Windows scales by the system font (about 2 px each), so a fresh install opened a roughly 1600x800 window (wider than a 1512 px screen) where macOS and Linux open 800x400. The window now opens at 800x400 (scaled by the display's DPI); a saved position and size still restore as before.
- **Saving a Standalone file that is also on the timeline failed** - REAPER keeps the source file of every take open, and Windows refuses to overwrite a held file, so Ctrl+S (and the save inside "Replace Source in REAPER Timeline") over a file the project uses ended in "Failed to save file". The items that use the file are now taken offline for the write and put back afterwards, like the in-place edits.
- **The SneakPeak window had no caption until the first item loaded** - the window title (and the taskbar entry) was empty after opening; it is now "SneakPeak" from the start, as on macOS and Linux.

#### Linux
- **The support links in the right-click menu did nothing** - they called the macOS opener (`/usr/bin/open`); they now go through `xdg-open` on Linux.

#### WAV files
- **WAVE_FORMAT_EXTENSIBLE files are read for what they are** - a WAV whose header uses the EXTENSIBLE format tag (many DAWs and field recorders write 24-bit and 32-bit float this way) was refused by the in-place editors (Reverse, DC Remove, Gain on a selection) and drag-exported as 24-bit; the real sample format is now taken from the header's sub-format.
- **Streamed WAVs with no data size edit correctly** - a WAV whose data chunk says 0 or "unknown" (a recording that was never finalized) reported a successful edit without changing a sample; the length now comes from the file itself.
- **Saving no longer shifts every sample by one step** - 16-bit and 24-bit PCM were written with a truncating encoder: a load -> save round trip moved almost every value one step towards zero and clipped -1.0 one step short. Samples now round to the nearest grid value and clamp exactly, so an unedited part of a file is written back bit for bit (in-place edits, Standalone saves and exports alike).
- **Standalone saves keep the original WAV's metadata** - bext (Broadcast WAV), iXML, LIST/INFO, cue and every other chunk of the original file are written back verbatim after the audio (Save and Save As).
- **A loop written after an odd-length data chunk was unreadable** - the smpl chunk now starts on an even boundary and the RIFF size counts every chunk; a drag export longer than the 4 GB WAV limit is refused with a message instead of producing a broken file.

### Known limits
- **Standalone is mono/stereo** - files with more than two channels are refused (edit them as items, where the in-place editors keep every channel); the working buffer is capped at 1 GB.
- **Ripple is single-track** - Ripple Delete (Shift+Delete; always in SET view) and Paste shift the following items on the edited track only; markers, regions, envelopes and other tracks are not moved.
- **Items over about 3.8 minutes (stereo, 44.1 kHz)** - the RMS band stays off, the Hard Limiter refuses them, and the One-Shot Factory skips a slice longer than that.
- **Reversed and sectioned takes** are refused by the destructive edits - reverse or unsection the take in REAPER first, or glue it.
- **Cue points are copied as they were** - a Standalone save of a shortened file may keep cue points past its end.
...
### Fixed
- **Dynamics sliders no longer stutter on multi-minute items** - every knob move re-ran the full peak/RMS scan over the whole file; the scan is now cached and re-runs only when the audio or the detection mode actually changes (reported by BogdanS on a 4-minute item). Very long items (15+ minutes) can still feel heavy in the panel - a separate bottleneck tracked for the next release.
