...
- **BOTH mode (leveler)** - the mode button cycles DOWN, UP, BOTH: in BOTH the compressor reduces above the threshold AND boosts below it in a single pass, pulling everything toward the threshold (the classic "lift the quiet, tame the loud" voice leveler; the two knee curves join into an exactly straight line through the threshold). The boost side keeps the M.Boost cap and the gate floor.
