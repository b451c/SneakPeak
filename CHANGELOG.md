...
- **Windows: a destructive edit right after a source swap or a fresh insert failed with an error box** - REAPER keeps a source file open while its peak builder reads it and for a moment after the item comes online, so an in-place edit started in that window (the Hard Limiter straight after "convert MP3 to WAV", Reverse on a file just dropped in) could not open the file and ended in "Write failed and the file could NOT be restored" although nothing had been written. The edit now waits for REAPER to let go of the file (up to 15 s, Esc cancels, "0%" in the title meanwhile); a file still held afterwards is left untouched and the toast says so.
