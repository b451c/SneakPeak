...
- **Apply Dynamics on long items no longer stalls REAPER** - the curve written to the envelope is simplified from every analysis point (3.6 million for an hour of audio), and that simplification ran on the main thread when Apply was pressed; it now runs on the analysis worker right after the analysis, so Apply and Live writes only insert the ready curve. Measured on a one-hour WAV: longest main-thread stall on Apply 0.70 s -> 0.05 s, identical points. A click or key while that worker is still busy no longer waits for it on item views (it only ever needed to on Standalone, where the worker reads the editable buffer).
- **Apply Dynamics keeps the envelope under 20,000 points** - an hour of busy material simplified to about 30,000 points at the 0.3 dB tolerance, more than REAPER's envelope editing handles comfortably; the curve is now coarsened step by step (up to a 2 dB bound) until it is under budget, and the toast reports the resulting tolerance ("Envelope simplified to N points (tolerance X dB)"). Short and normal items are unchanged.
- **De-esser analysis no longer grinds through long silences on x86** - the sidechain filter's state never reaches zero in a silence by itself (it settles on the smallest representable number), and x86 processors handle such values through slow microcode, so the background analysis of material with long pauses could take many times longer than it should. The filter state is now flushed to zero below -600 dB and the analysis thread runs with flush-to-zero set; the envelope output is unchanged (the raw sidechain values below 1e-30 were already floored before use).
...
- **Standalone Dynamics could keep analysing the audio from before a small edit** - the analysis is cached behind a sparse fingerprint of the buffer (every 4096th sample), so a short in-place edit that fell between fingerprinted samples (a silenced syllable, a pencil stroke) left the old analysis in place and the next Apply compressed with the curve of the unedited audio. Every Standalone edit now drops the cached analysis outright.
...
- **Standalone saves keep the original WAV's metadata** - bext (Broadcast WAV), iXML, LIST/INFO, cue and every other chunk of the original file are written back verbatim after the audio. Cue points are copied as they were, so a shortened file may keep cue points past its end.
...
- **Snap to Zero-Crossing did nothing on long items** - the snap searched the working buffer, which an item over about 3.8 minutes (stereo, 44.1 kHz) no longer has (it is decoded only when something needs the samples) or holds at a reduced rate, so the selection edges stayed where the mouse was (or on the reduced grid). The edges are now looked up in a small window read from the source at its own rate: sample-accurate on items of any length.
